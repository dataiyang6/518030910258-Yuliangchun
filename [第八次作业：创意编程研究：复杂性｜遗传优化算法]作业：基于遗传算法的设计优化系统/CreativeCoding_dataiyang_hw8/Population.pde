...
class Population {
  ...
  Population(float m, int num) {
    ...
    for (int i = 0; i < population.length; i++) {
      population[i] = new Design(new DNA(), 20+(i%(population.length/2))*220, 50 + ceil(i/(population.length/2))*240);
    }
  }
  ...
  void reproduction() {
    ...
    for (int i = 0; i < population.length; i++) {
      ...
      population[i] = new Design(child, 20+(i%(population.length/2))*220, 50 + ceil(i/(population.length/2))*240);
    }
    generations++;
  }
